...
for oldfile in glob.glob( os.path.join('./', '*_charm_toGordon.txt') ):
    ...
    if line.find(",charm,")!=-1:
        ...
        newfile = oldfile
        newfile = newfile.replace("_charm_toGordon.txt","_tau_toGordon.txt")
        print "          to " + newfile
...
#finally create new files
        for i in range(stop):
            ...
            if line.find("}") != -1:
                outfile.write('                sys(extrapolation from c to tau,22%) \n');
            outfile.write(line)
        outfile.write('}\n')    
